...
__export(index_exports, {
  ...
  OAuth2UserAuthSerializer: () => OAuth2UserAuthSerializer,
...
var JsonSchemaSchema = import_zod2.z.lazy(() => import_zod2.z.object({
  ...
  examples: import_zod2.z.array(JsonTypeSchema).optional(),
...
// src/version.ts
var _VERSION = "1.1.1";
var LIB_VERSION = _VERSION === "1.1.1" ? "1.0.0" : _VERSION;
...
// src/data/auth_implementations/oauth2_user_auth.ts
var import_zod8 = require("zod");
var OAuth2UserAuthSerializer = class extends Serializer {
  toDict(obj) {
    return { ...obj };
  }
  validateDict(obj) {
...
var OAuth2UserAuthSchema = import_zod8.z.object({
  auth_type: import_zod8.z.literal("oauth2_user"),
  access_token: import_zod8.z.string().describe('Runtime bearer token. Recommended to use an injected variable like "${MY_TOKEN}".'),
  grant_type: import_zod8.z.enum(["device_code", "authorization_code"]).optional().describe("Interactive flow the login tool should run."),
  token_endpoint: import_zod8.z.string().optional().describe("OAuth2 token endpoint (discovered for MCP manuals)."),
  client_id: import_zod8.z.string().optional().describe("Public OAuth2 client id (omit for MCP dynamic client registration)."),
  scope: import_zod8.z.string().optional().describe("Optional requested scope."),
  device_authorization_endpoint: import_zod8.z.string().optional().describe("Device authorization endpoint \u2014 required for the device_code flow."),
  authorization_endpoint: import_zod8.z.string().optional().describe("Authorization endpoint \u2014 required for the authorization_code flow."),
  var_name: import_zod8.z.string().optional().describe('Header name to inject the token into. Defaults to "Authorization".'),
  prefix: import_zod8.z.string().optional().describe('Token prefix. Defaults to "Bearer ".')
});
...
var InMemConcurrentToolRepositorySerializer = class extends Serializer {
  ...
  validateDict(data) {
    try {
      ...
      if (e instanceof import_zod9.z.ZodError) {
...
var InMemConcurrentToolRepositoryConfigSchema = import_zod9.z.object({
  tool_repository_type: import_zod9.z.literal("in_memory")
}).passthrough();
...
var ConcurrentToolRepositorySchema = import_zod10.default.custom((obj) => {
  try {
...
var TagSearchStrategyConfigSerializer = class extends Serializer {
  ...
  validateDict(data) {
    try {
      ...
      if (e instanceof import_zod11.z.ZodError) {
        throw new Error(`Invalid configuration: ${e.message}`);
      }
      throw new Error("Unexpected error during validation");
    }
...
var TagSearchStrategyConfigSchema = import_zod11.z.object({
  tool_search_strategy_type: import_zod11.z.literal("tag_and_description_word_match"),
  description_weight: import_zod11.z.number().optional().default(1),
  tag_weight: import_zod11.z.number().optional().default(3)
}).passthrough();
...
var ToolSearchStrategySchema = import_zod12.default.custom((obj) => {
  try {
...
var FilterDictPostProcessorConfigSchema = import_zod13.z.object({
  tool_post_processor_type: import_zod13.z.literal("filter_dict"),
  exclude_keys: import_zod13.z.array(import_zod13.z.string()).optional(),
  only_include_keys: import_zod13.z.array(import_zod13.z.string()).optional(),
...
var LimitStringsPostProcessorConfigSchema = import_zod14.z.object({
  tool_post_processor_type: import_zod14.z.literal("limit_strings"),
  limit: import_zod14.z.number().int().positive().default(1e4),
  exclude_tools: import_zod14.z.array(import_zod14.z.string()).optional(),
  only_include_tools: import_zod14.z.array(import_zod14.z.string()).optional(),
  exclude_manuals: import_zod14.z.array(import_zod14.z.string()).optional(),
  only_include_manuals: import_zod14.z.array(import_zod14.z.string()).optional()
}).passthrough();
var LimitStringsPostProcessorSerializer = class extends Serializer {
  ...
  validateDict(data) {
    try {
      ...
      if (e instanceof import_zod14.z.ZodError) {
        throw new Error(`Invalid configuration: ${e.message}`);
      }
      throw new Error("Unexpected error during validation");
    }
...
var ToolPostProcessorSchema = import_zod15.default.custom((obj) => {
  try {
...
var VariableLoaderSchema = import_zod16.z.custom((obj) => {
  try {
...
var UtcpClientConfigSchema = import_zod17.z.object({
  variables: import_zod17.z.record(import_zod17.z.string(), import_zod17.z.string()).optional().default({}),
  load_variables_from: import_zod17.z.array(VariableLoaderSchema).nullable().optional().default(null).transform((val) => {
  ...
  tool_repository: import_zod17.z.any().transform((val) => {
  ...
  tool_search_strategy: import_zod17.z.any().transform((val) => {
  ...
  post_processing: import_zod17.z.array(import_zod17.z.any()).transform((val) => {
    return val.map((item) => {
  ...
  manual_call_templates: import_zod17.z.array(CallTemplateSchema).transform((val) => {
    return val.map((item) => {
...
// src/client/utcp_client.ts
var UtcpClient = class _UtcpClient {
