...
__export(index_exports, {
  HttpCallTemplateSchema: () => HttpCallTemplateSchema,
  HttpCallTemplateSerializer: () => HttpCallTemplateSerializer,
  HttpCommunicationProtocol: () => HttpCommunicationProtocol,
  OpenApiConverter: () => OpenApiConverter,
  SseCallTemplateSchema: () => SseCallTemplateSchema,
  SseCallTemplateSerializer: () => SseCallTemplateSerializer,
  SseCommunicationProtocol: () => SseCommunicationProtocol,
  StreamableHttpCallTemplateSchema: () => StreamableHttpCallTemplateSchema,
  StreamableHttpCallTemplateSerializer: () => StreamableHttpCallTemplateSerializer,
  StreamableHttpCommunicationProtocol: () => StreamableHttpCommunicationProtocol,
  register: () => register
});
module.exports = __toCommonJS(index_exports);
var import_sdk12 = require("@utcp/sdk");
...
// src/http_call_template.ts
var import_sdk = require("@utcp/sdk");
var import_sdk2 = require("@utcp/sdk");
var HttpCallTemplateSchema = external_exports.object({
  name: external_exports.string().optional(),
  call_template_type: external_exports.literal("http"),
  http_method: external_exports.enum(["GET", "POST", "PUT", "DELETE", "PATCH"]).default("GET"),
  url: external_exports.string().describe('The base URL for the HTTP endpoint. Supports path parameters like "https://api.example.com/users/{user_id}".'),
  content_type: external_exports.string().default("application/json").describe("The Content-Type header for requests."),
  auth: import_sdk.AuthSchema.optional().describe("Optional authentication configuration."),
  headers: external_exports.record(external_exports.string(), external_exports.string()).optional().describe("Optional static headers to include in all requests."),
  body_field: external_exports.string().optional().default("body").describe("The name of the single input field to be sent as the request body."),
  header_fields: external_exports.array(external_exports.string()).optional().describe("List of input fields to be sent as request headers."),
  auth_tools: import_sdk.AuthSchema.nullable().optional().transform((val) => {
...
// src/streamable_http_call_template.ts
var import_sdk3 = require("@utcp/sdk");
var import_sdk4 = require("@utcp/sdk");
var StreamableHttpCallTemplateSchema = external_exports.object({
  name: external_exports.string().optional(),
  call_template_type: external_exports.literal("streamable_http"),
  url: external_exports.string().describe("The streaming HTTP endpoint URL. Supports path parameters."),
  http_method: external_exports.enum(["GET", "POST"]).default("GET"),
  content_type: external_exports.string().default("application/octet-stream").describe("The Content-Type header for requests."),
  chunk_size: external_exports.number().default(4096).describe("Size of each chunk in bytes for reading the stream."),
  timeout: external_exports.number().default(6e4).describe("Request timeout in milliseconds."),
...
// src/sse_call_template.ts
var import_sdk5 = require("@utcp/sdk");
var import_sdk6 = require("@utcp/sdk");
var SseCallTemplateSchema = external_exports.object({
  name: external_exports.string().optional(),
  call_template_type: external_exports.literal("sse"),
  url: external_exports.string().describe("The SSE endpoint URL to connect to."),
  event_type: external_exports.string().nullable().optional().describe("Optional filter for specific event types. If null, all events are received."),
  reconnect: external_exports.boolean().default(true).describe("Whether to automatically reconnect on connection loss."),
  retry_timeout: external_exports.number().default(3e4).describe("Timeout in milliseconds before attempting reconnection."),
...
var import_sdk7 = require("@utcp/sdk");
var import_sdk8 = require("@utcp/sdk");
...
function isIpLoopbackLike(host) {
...
function ensureSecureUrl(url, context) {
  if (isSecureUrl(url)) return;
  const where = context ? ` during ${context}` : "";
  throw new Error(
    `Security error${where}: URL must use HTTPS or be a literal loopback address (localhost / 127.0.0.1 / ::1). Got: ${JSON.stringify(url)}. Plain HTTP to any other host is rejected to prevent MITM attacks and SSRF into internal services.`
  );
}
function assertNoCrlf(value, fieldName) {
  if (typeof value !== "string") return;
  if (value.includes("\r") || value.includes("\n")) {
    throw new Error(
      `Refusing to construct request: ${fieldName} contains CR/LF, which would enable HTTP header injection.`
    );
  }
}
var REDIRECT_STATUSES = /* @__PURE__ */ new Set([301, 302, 303, 307, 308]);
var AUTH_SENSITIVE_HEADERS = /* @__PURE__ */ new Set([
...
var AUTH_HEADER_REGEX = /(?:(?:^|[-_])(?:auth|authn|authz|token|key|secret|bearer|session|sid|api[-_]?key|jwt|csrf|xsrf)(?:[-_]|$))|(?:apikey|authtoken|accesstoken|bearertoken|sessionid|csrftoken|xsrftoken|xapikey|xauthtoken|xaccesstoken|xapitoken)/i;
function headerIsAuthSensitive(name) {
...
function scrubCrossOriginCredentials(config, extraAuthHeaderNames = /* @__PURE__ */ new Set()) {
...
async function safeRequestWithRedirects(axiosInstance, config, context, maxHops = 5, authHeaderNames = []) {
  ...
  const extraAuthHeaderNames = new Set(
    authHeaderNames.filter((n) => typeof n === "string").map((n) => n.toLowerCase())
  );
...
// src/openapi_converter.ts
var OPENAPI_OPERATION_METHODS = ["get", "put", "post", "delete", "options", "head", "patch", "trace"];
var SUPPORTED_HTTP_METHODS = ["GET", "POST", "PUT", "DELETE", "PATCH"];
var OpenApiConverter = class {
  ...
  convert() {
    ...
    if (!baseUrlIsExplicitOverride && this.spec_url && !isLoopbackUrl(this.spec_url) && isLoopbackUrl(baseUrl)) {
      throw new Error(
        `Security error: OpenAPI spec fetched from ${JSON.stringify(this.spec_url)} declares a loopback base URL (${JSON.stringify(baseUrl)}). A remote spec is not allowed to redirect tool calls at the agent's own loopback interface \u2014 this is the SSRF pattern from GHSA-39j6-4867-gg4w / CVE-2026-44661. If you trust this spec, set the call template's \`base_url\` override explicitly to bypass this check.`
      );
    }
    const paths = this.spec.paths || {};
    for (const [path, pathItem] of Object.entries(paths)) {
      for (const [method, operation] of Object.entries(pathItem)) {
        if (OPENAPI_OPERATION_METHODS.includes(method.toLowerCase())) {
    ...
    return new import_sdk8.UtcpManualSerializer().validateDict({ tools });
  }
  /**
   ...
   */
  _resolveSchema(schema, visitedRefs = /* @__PURE__ */ new Set()) {
  ...
  _extractExamples(obj) {
    ...
    const examples = [];
    if ("example" in obj && obj.example !== void 0 && obj.example !== null) {
    ...
    if ("examples" in obj && obj.examples) {
      if (Array.isArray(obj.examples)) {
        ...
        for (let exampleObj of Object.values(obj.examples)) {
   ...
   */
  _schemaWithoutExampleKeys(schema) {
    const out = {};
    for (const [key, value] of Object.entries(schema)) {
  ...
  _createTool(path, method, operation, baseUrl) {
    ...
    return {
      name: operationId,
      description,
      inputs: import_sdk7.JsonSchemaSchema.parse(inputs),
      outputs: import_sdk7.JsonSchemaSchema.parse(outputs),
      tags,
  ...
  _extractOutputs(operation) {
    ...
    let jsonSchema = null;
    let mediaTypeObj = null;
    if ("content" in resolvedResponse) {
    ...
    } else if ("schema" in resolvedResponse) {
    ...
    const schemaArgs = {
      ...
      properties: resolvedJsonSchema.properties || void 0,
      required: resolvedJsonSchema.required || void 0,
      description: resolvedJsonSchema.description || void 0,
      title: resolvedJsonSchema.title || void 0,
      items: resolvedJsonSchema.items || void 0,
      enum: resolvedJsonSchema.enum || void 0,
      minimum: resolvedJsonSchema.minimum || void 0,
      maximum: resolvedJsonSchema.maximum || void 0,
      format: resolvedJsonSchema.format || void 0
    };
    if (responseExamples) {
      ...
      if (Object.keys(flows).length > 0) {
        for (const [flowType, flowConfig] of Object.entries(flows)) {
          if (["authorizationCode", "accessCode", "clientCredentials", "application"].includes(flowType)) {
            const tokenUrl2 = flowConfig.tokenUrl;
            if (tokenUrl2) {
              const resolvedTokenUrl = this._validateTokenUrlEagerly(tokenUrl2);
              ...
              return {
                ...
                scope: Object.keys(scopes).length > 0 ? Object.keys(scopes).join(" ") : void 0
              };
            }
          }
        }
   ...
   *     reject it later.
   */
  _validateTokenUrlEagerly(tokenUrl) {
    let absoluteParse = null;
    try {
...
var HttpCommunicationProtocol = class {
  ...
  async registerManual(caller, manualCallTemplate) {
    ...
    try {
      ...
      if (responseData && responseData.utcp_version && Array.isArray(responseData.tools)) {
        ...
        utcpManual = import_sdk9.UtcpManualSchema.parse(responseData);
      ...
      return {
        manualCallTemplate: httpCallTemplate,
        manual: import_sdk9.UtcpManualSchema.parse({ tools: [] }),
        success: false,
        errors: [import_axios.default.isAxiosError(error) ? error.message : String(error)]
      };
    }
  }
  /**
  ...
  async callTool(caller, toolName, toolArgs, toolCallTemplate) {
    ...
    const { authHeaderNames } = await this._applyAuthToRequestConfig(httpCallTemplate, requestConfig);
    try {
      if (bodyContent !== void 0 && !("Content-Type" in (requestConfig.headers || {}))) {
  ...
  async _applyAuthToRequestConfig(httpCallTemplate, requestConfig) {
    ...
    if (httpCallTemplate.auth) {
      if (httpCallTemplate.auth.auth_type === "api_key") {
        ...
        const userAuth = httpCallTemplate.auth;
    ...
    tokenFetchPromises.push(
      (async () => {
        try {
          ...
          const expiresAt = Date.now() + (response.data.expires_in * 1e3 || 3600 * 1e3);
...
// src/streamable_http_communication_protocol.ts
var import_sdk10 = require("@utcp/sdk");
var StreamableHttpCommunicationProtocol = class {
  oauthTokens = /* @__PURE__ */ new Map();
  _logInfo(message) {
  ...
  _applyAuth(provider, headers, queryParams) {
    ...
    if (provider.auth) {
      if ("api_key" in provider.auth) {
        ...
        const userAuth = provider.auth;
        if (!userAuth.access_token) {
          throw new Error(
            "access_token for oauth2_user auth is empty. Run an interactive login to provision it."
          );
        }
        const headerName = userAuth.var_name || "Authorization";
        const prefix = userAuth.prefix ?? "Bearer ";
  ...
  async registerManual(caller, manualCallTemplate) {
    ...
    try {
      ...
      const utcpManual = new import_sdk10.UtcpManualSerializer().validateDict(JSON.parse(responseText));
      ...
      return {
        manualCallTemplate: provider,
        manual: new import_sdk10.UtcpManualSerializer().validateDict({ tools: [] }),
        success: false,
...
// src/sse_communication_protocol.ts
var import_sdk11 = require("@utcp/sdk");
var SseCommunicationProtocol = class {
  oauthTokens = /* @__PURE__ */ new Map();
  _logInfo(message) {
  ...
  async registerManual(caller, manualCallTemplate) {
    ...
    try {
      ...
      const responseText = await response.text();
      const utcpManual = new import_sdk11.UtcpManualSerializer().validateDict(JSON.parse(responseText));
      this._logInfo(`Discovered ${utcpManual.tools.length} tools from '${provider.name}'`);
      return {
        manualCallTemplate: provider,
        manual: utcpManual,
      ...
      return {
        manualCallTemplate: provider,
        manual: new import_sdk11.UtcpManualSerializer().validateDict({ tools: [] }),
        success: false,
        errors: [error.message || String(error)]
      };
    }
...
function register(override = false) {
  (0, import_sdk12.ensureCorePluginsInitialized)();
  import_sdk12.CallTemplateSerializer.registerCallTemplate("http", new HttpCallTemplateSerializer(), override);
  import_sdk12.CommunicationProtocol.communicationProtocols["http"] = new HttpCommunicationProtocol();
  import_sdk12.CallTemplateSerializer.registerCallTemplate("streamable_http", new StreamableHttpCallTemplateSerializer(), override);
  import_sdk12.CommunicationProtocol.communicationProtocols["streamable_http"] = new StreamableHttpCommunicationProtocol();
  import_sdk12.CallTemplateSerializer.registerCallTemplate("sse", new SseCallTemplateSerializer(), override);
  import_sdk12.CommunicationProtocol.communicationProtocols["sse"] = new SseCommunicationProtocol();
}
register();
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  HttpCallTemplateSchema,
  HttpCallTemplateSerializer,
  HttpCommunicationProtocol,
  OpenApiConverter,
  SseCallTemplateSchema,
  SseCallTemplateSerializer,
  SseCommunicationProtocol,
  StreamableHttpCallTemplateSchema,
